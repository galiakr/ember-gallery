  ...
  test('it renders', async function (assert) {
    ...
    this.set('externalActionNext', (actual) => {
      ...
      assert.deepEqual(
        actual,
        expected,
        ' next - is passed to external action'
      );
    });
    ...
    this.set('externalActionPrev', (actual) => {
      ...
      assert.deepEqual(
        actual,
        expected,
        'prev - value is passed to external action'
      );
    });

    await render(
      hbs`<Pagination @activePage={{activePage}} @onChangePage={{this.externalActionNext}} />`
    );
    await click('[data-test-next-page]');

    await render(
      hbs`<Pagination @activePage={{activePage}} @onChangePage={{this.externalActionPrev}} />`
    );
